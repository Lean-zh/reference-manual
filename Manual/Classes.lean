...
在 Lean 中，多态有三种变体：
 ...
 1. {tech (key := "universe polymorphism")}[宇宙多态]，其中定义中的 Sort 可以用各种方式实例化，
...
以下是类型类的一些典型用例：
 * 类型类可以表示重载运算符，比如可用于多种数值类型的算术运算符，或者可用于多种数据结构的成员判定谓词。对于给定类型，运算符通常有一个唯一的规范选择——毕竟对于 {lean}`Nat` 的加法没有其他合理的替代定义——但这并非一个必然属性，库如果需要也可以提供替代实例。
 ...
 * 类型类可以表示类型制导的代码生成框架，其中多态类型的实例各自贡献最终程序的一部分。
    ...
    当美观打印最终在已知具体类型的表达式（如 {lean}`List (Nat × (String ⊕ Int))`）上被调用时，产生的美观打印器将包含由 {name}`List`、{name}`Prod`、{name}`Nat`、{name}`Sum`、{name}`String` 和 {name}`Int` 的 {name}`Repr` 实例组装而成的代码。
