...
* [允许在元组、列表和策略中使用尾随逗号](https://github.com/leanprover/lean4/pull/2643)。
...
* Lake 现在会过滤从已编译配置（`lakefile.olean`）中加载的环境扩展，只保留与 Lake 工作区加载流程有关的部分。这解决了因环境扩展类型不匹配而导致的段错误（例如在配置中通过 `elab` 定义自定义精译器时）。参见 issue [#2632](https://github.com/leanprover/lean4/issues/2632) 与 PR [#2896](https://github.com/leanprover/lean4/pull/2896)。
