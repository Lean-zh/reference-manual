...
* [#8449](https://github.com/leanprover/lean4/pull/8449) 和 [#8516](https://github.com/leanprover/lean4/pull/8516)
  将 Mathlib 的 `clear_value` tactic 上游化并加以扩展。给定一个
  局部定义 `x : T := v`，`clear_value x` tactic 会将其替换为
...
* [#8450](https://github.com/leanprover/lean4/pull/8450) 为 `subst` tactic 增加了一项功能：当 `x : X := v`
...
* [#8104](https://github.com/leanprover/lean4/pull/8104) 让 `fun_induction` 和 `fun_cases`（尝试）
  ...
  参见 issue [#8296](https://github.com/leanprover/lean4/issues/8296)。
...
* [#8106](https://github.com/leanprover/lean4/pull/8106) 新增 `register_linter_set` 命令，用于声明 linter 集合。
  `getLinterValue` 函数现在会检查当前 linter 是否
...
* [#7352](https://github.com/leanprover/lean4/pull/7352) 重做了围绕 `Id` monad 的 `simp` 集，
...
* [#8002](https://github.com/leanprover/lean4/pull/8002) 修复了一个问题：对被 `induction` 和 `cases`
  tactic 泛化出来的变量执行“跳转到定义”时无法正常工作。关闭了
  #2873。

* [#8024](https://github.com/leanprover/lean4/pull/8024) 为 `lean` CLI 添加了 `--setup` 选项。它接受一个
  JSON 文件路径，其中包含模块的 imports 和
  配置信息，并以此覆盖模块自身文件头中的对应信息。Lake 将使用它来
  指定模块产物（例如 oleans 和 ileans）的路径，
  并将其与 `LEAN_PATH` 方案分离。
...
* [#8196](https://github.com/leanprover/lean4/pull/8196) 改进了 `grind` 中 E-matching 模式推断的过程。
  考虑下面这个定理：
  ```lean
  @[grind →]
...
* [#8209](https://github.com/leanprover/lean4/pull/8209) 修复了 `grind` tactic 中的一个非确定性问题。
  这是模型驱动理论组合模块中的一个 bug。
...
* [#8231](https://github.com/leanprover/lean4/pull/8231) 改变了 `apply?` 的行为，使它用于
  ...
  sorry 要求 tactic 同时生成一条错误消息，而在这个场景下我们并不希望如此。）
  这一改动可防御 [#8212](https://github.com/leanprover/lean4/issues/8212) 中报告的问题。

* [#8232](https://github.com/leanprover/lean4/pull/8232) 修复了 `rewrite` tactic 中常量的 elaboration。
  此前，`rw [eq_self]` 会对 `eq_self` 做两次 elaboration，并把它
  在 infotree 中加入两次。这会导致 “Expected type”
  在反展开时带着一个未知的 universe metavariable。

* [#8241](https://github.com/leanprover/lean4/pull/8241) 修改了 `rename` tactic 的行为，使其在查找要重命名的假设时跳过
  实现细节性质的假设。
...
* [#8259](https://github.com/leanprover/lean4/pull/8259) 在被投影值的类型是 metavariable 时，
...
* [#8262](https://github.com/leanprover/lean4/pull/8262) 改进了 type-as-hole 错误消息。针对
  theorem 声明的 type-as-hole 错误，不应暗示可以完全省略类型。
...
* [#8270](https://github.com/leanprover/lean4/pull/8270) 让 `bv_decide` 的 enum pass 能处理
  universe 多态的枚举类型。

* [#8271](https://github.com/leanprover/lean4/pull/8271) 修改了 `addPPExplicitToExposeDiff`，使其显示 universe 差异，
  并深入访问 projection，例如：
  ```
...
* [#8277](https://github.com/leanprover/lean4/pull/8277) 通过更可靠地重写 `match` 语句，改进了
  `.induct_unfolding` 的生成；其做法是使用 #8284 中引入的新“congruence equations”。
  修复了 #8195。
...
* [#8284](https://github.com/leanprover/lean4/pull/8284) 为 matcher 增加了一类新的方程，即
  “congruence equations”，它推广了普通 matcher 方程。它们具有
  不受限制的左侧、把 discriminant 与 pattern 联系起来的额外等式假设，
  因而可以证明异构等式。从这个意义上说，它们把同余与重写结合了起来。
  它们可用于重写 matcher 应用，尤其是在依赖关系存在时 `simp`
  无法重写 discriminant 的情形，并将用于生成 unfolding 归纳定理。

* [#8285](https://github.com/leanprover/lean4/pull/8285) 修复了为带命名 pattern 的 match 语句生成
  splitter 时出现的 “declaration has free variables” 错误。修复了 #8274。

* [#8299](https://github.com/leanprover/lean4/pull/8299) 在 `grind` 中实现了一个缺失的预处理步骤：
  对目标中的 metavariable 做抽象化。
...
* [#8302](https://github.com/leanprover/lean4/pull/8302) 让 `cases` 在 motive 含有复杂参数、
  且该参数类型依赖于目标时能够优雅失败。`induction` tactic 能较好处理这种情况，
  但 `cases` 不能。这个改动至少能优雅退化为不实例化那个 motive 参数。
  更多细节参见 issue [#8296](https://github.com/leanprover/lean4/issues/8296)。
...
* [#8361](https://github.com/leanprover/lean4/pull/8361) 修复了 #3188 中引入的 `cases` tactic bug：
  当 `cases`（不是 `induction`）配合 `using` 的非原子表达式使用时，
  参数索引会混乱。

* [#8363](https://github.com/leanprover/lean4/pull/8363) 以无冲突的方式统一了各种辅助声明的命名方法，
  并确保该方法兼容 elaboration 的分叉分支，例如并行化或类似 Aesop 的
  回溯加重放搜索。

* [#8365](https://github.com/leanprover/lean4/pull/8365) 修复了 ground pattern 的透明度模式。
...
* [#8368](https://github.com/leanprover/lean4/pull/8368) 改进了无效 pattern-match
  分支产生的错误消息，并提升了模式匹配 tactic 与 elaborator 在错误位置上的一致性。
...
* [#8426](https://github.com/leanprover/lean4/pull/8426) 新增属性 `[grind?]`。它与 `[grind]` 类似，
  但会显示推断得到的 E-matching 模式。比手工书写更方便。
  感谢 @kim-em 提出这一功能建议。
  ```lean
...
* [#8429](https://github.com/leanprover/lean4/pull/8429) 添加了 `Lean.Grind.Ring.IsOrdered`，并清理了
  ring/module 的 `grind` API。这些类型类目前尚未被使用，但会支撑
  `grind` 未来的算法改进。

* [#8437](https://github.com/leanprover/lean4/pull/8437) 修复了目标中存在 metavariable 时 `split`
  的行为。
...
* [#8440](https://github.com/leanprover/lean4/pull/8440) 为 `grind`
  tactic 实现了非按时间顺序的回溯。这一特性确保 `grind`
  在执行了一个并不相关的 case split 之后，不需要继续处理无关分支。
...
* [#8449](https://github.com/leanprover/lean4/pull/8449) 将 Mathlib 的 `clear_value` tactic 上游化并加以扩展。给定一个
  局部定义 `x : T := v`，`clear_value x` tactic 会将其替换为
  一个假设 `x : T`；如果目标不依赖值 `v`，则会报错。
...
* [#8506](https://github.com/leanprover/lean4/pull/8506) 在 `grind` 中借助 `match`
  congruence equations 实现了 `match` 表达式。目标是尽量减少
...
* [#8544](https://github.com/leanprover/lean4/pull/8544) 在 `grind` tactic 中实现了对过量应用
  `ite` 和 `dite` 的支持，并增加了传播和 case split 的支持。
...
* [#8564](https://github.com/leanprover/lean4/pull/8564) 简化了 `grind` 核心与 cutsat
  ...
  需要在 cutsat 中内部化的数字字面量数量。这个优化有 bug
...
* [#8182](https://github.com/leanprover/lean4/pull/8182) 为所有 hash/tree map 类型增加了
...
* [#8315](https://github.com/leanprover/lean4/pull/8315) 将 `Std.Classes.Ord` 拆分为
  `Std.Classes.Ord.Basic`（只带少量 imports）、`Std.Classes.Ord.SInt`
...
* [#8620](https://github.com/leanprover/lean4/pull/8620) 移除了 `NatCast (Fin n)` 全局实例（包括
  ...
  `x < n`（其中 `x : Fin k`、`n : Nat`）被 elaboration 为
...
* [#8308](https://github.com/leanprover/lean4/pull/8308) 让新编译器的 specialization pass
...
* [#8462](https://github.com/leanprover/lean4/pull/8462) 默认启用了 LCNF 的 extractClosed pass。
...
* [#8502](https://github.com/leanprover/lean4/pull/8502) 让新编译器改用内核环境查找定义，
  因而当某个声明带有内核错误（例如含有未解决的 metavariable）时，会跳过编译。
  这与旧编译器的行为一致。
...
* [#8540](https://github.com/leanprover/lean4/pull/8540) 修改了 LCNF specialize pass，
  允许 ground 变量依赖局部函数声明（只要没有非 ground 的自由变量）。
  这使得依赖局部 lambda 的 Monad 实例也能做 specialization。
...
* [#8565](https://github.com/leanprover/lean4/pull/8565) 让 LCNF specialization pass 只把类型/实例参数
...
* [#8041](https://github.com/leanprover/lean4/pull/8041) 修改了 `pp.showLetValues` 的行为，
  ...
  对于 tactic metavariable，还有额外的选项
  `pp.showLetValues.tactic.threshold`，其默认值设置为最大值，
  因为在 tactic state 中局部值通常很重要。
...
* [#8362](https://github.com/leanprover/lean4/pull/8362) 修复了一个 bug：某些
...
* [#8411](https://github.com/leanprover/lean4/pull/8411) 修复了 Resolve.lean 中的一个文档 bug；
  在逆序中，B 应排在 A 前面。
...
* [#8529](https://github.com/leanprover/lean4/pull/8529) 修改了 `lake lean` 和 `lake setup-file`，
  使其会使用 import 对应的整个库，来预编译非工作区文件的 imports。
  这样能确保额外的链接对象在 elaboration 期间已被链接并可用。
