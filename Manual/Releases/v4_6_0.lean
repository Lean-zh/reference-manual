...
* `termination_by` 与 `decreasing_by` 终止性提示的语法已全面调整：
  ...
  * `termination_by` 子句现在只能绑定“额外参数”，即那些并未在函数头中绑定，
  ...
  过去若一个 `decreasing_by` 子句会作用于多个互递归函数，现在必须将该 tactic 分别重复写出。

* `decreasing_by` 的语义已更改；现在 tactic 会一次性应用于所有终止性证明目标，而不是逐个应用。

  这使得交互式编写终止性证明更方便，因为现在可以分别聚焦每个子目标，例如使用 `·`。此前，给出的 tactic 脚本必须对 _所有_ 目标都有效，因此常常需要借助 `first` 之类的 tactic 组合子：
  ...
  如果要恢复旧行为、将某个 tactic 分别应用到每个目标，请使用 `all_goals`：
  ```diff
   def foo (n : Nat) := …
  ...
  对于互递归，现在每个 `decreasing_by` 只作用于它所属的函数。如果递归组中的某些函数没有自己的 `decreasing_by`，则会使用默认的 `decreasing_tactic`。如果多个函数需要应用同一个 tactic，就必须在这些函数处分别重复书写 `decreasing_by` 子句。

* 调整 `InfoTree.context`，以便在精化命令时更方便地为其补充局部上下文。这会破坏与下游项目的向后兼容性：凡是手动遍历 `InfoTree` 而不是通过 `InfoUtils.lean` 中函数访问的项目，以及手动创建并保存 `InfoTree` 的项目，都会受影响。如何迁移代码，请参见 [PR #3159](https://github.com/leanprover/lean4/pull/3159)。
