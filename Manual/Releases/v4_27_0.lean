/-
版权 (c) 2025 Lean FRO LLC.所有权利保留.
在Apache 2.0许可证下发布,如文件许可中描述的.
作者:安妮·巴恩
-/
...
[#11637](https://github.com/leanprover/lean4/pull/11637) 宣布模块系统不再属于实验性功能，并将 {option}`experimental.module` 选项变为 no-op。
...
- [#11162](https://github.com/leanprover/lean4/pull/11162) 降低了语言服务器的内存占用（尤其是 watchdog 进程）。在 Mathlib 中，内存占用大约减少了 1GB。
...
这一特性显著增强了 `grind` 对高阶相等式和部分应用函数相等式的支持；而在禁用 `funCC` 时，仍保持与一阶 SMT 行为的兼容性。
...
[#11268](https://github.com/leanprover/lean4/pull/11268) 实现了对任意 {tactic}`grind` 参数的支持。该特性与 {tactic}`simp` 中已有的机制类似：证明项会被视为局部的 universe-polymorphic 引理。此特性依赖于 `grind -revert`（见 [#11248](https://github.com/leanprover/lean4/pull/11248)）。例如，用户现在可以写出：
...
请注意，上面的例子里，`snd_eq` 只被实例化了两次，但使用了不同的 universe 参数。
...
这是一项与回退假设相关的内部变更。
采用新的默认值后，{tactic}`grind` 产生的 trace、反例和证明项都会不同。
...
本次发布完成了对 {name}`String` API 的修订，包括依赖类型化的 {name}`String.Pos`、对 {name}`String.Slice` 的完整 API 支持，以及采用新 `Iterator` API 的迭代器。
此外，{name}`TreeMap`/{name}`HashMap` API 也新增了许多内容，包括交集、差集与相等性。
...
- [#11490](https://github.com/leanprover/lean4/pull/11490) 防止 `try` 吞掉嵌套 `simp` 调用产生的 heartbeat 错误，更一般地说，它还确保 `isRuntime` 标志会被 `throwNestedTacticEx` 传播。
  这避免了证明的行为（尤其是使用 `aesop` 的证明）受当前递归深度或 heartbeat 限制影响。
...
* [#11196](https://github.com/leanprover/lean4/pull/11196) 在计算匹配器时跟踪可能的重叠、将信息存入 `MatcherInfo`，并在后续匹配器计算中复用这些信息，从而避免匹配分支器 为了检测重叠而对所有备选分支进行二次复杂度的两两测试。
...
* [#11200](https://github.com/leanprover/lean4/pull/11200) 更改了稀疏 case 表达式表示 “以上都不是” 信息的方式。
  ...
  这避免了 splitter 生成时的二次开销；原先对于 splitter 备选中的全部 n 个假设，都要通过 `.subst` 和 `.cases` 构造对这 n 个假设逐一精化。

* [#11221](https://github.com/leanprover/lean4/pull/11221) 让 `realizeConst` 使用 `withDeclNameForAuxNaming`，从而使其中创建的辅助定义获得不冲突的名字。

* [#11222](https://github.com/leanprover/lean4/pull/11222) 实现了 `elabToSyntax`，用于为任意 elaborator `el : Option Expr -> TermElabM Expr` 创建带作用域的语法 `s : Syntax`，满足 `elabTerm s = el`。

* [#11236](https://github.com/leanprover/lean4/pull/11236) 从 `Match.MatchEqs` 中抽取出两个模块，为 #11220 做准备，并借助模块系统更清晰地划分这里的关注点边界。

* [#11239](https://github.com/leanprover/lean4/pull/11239) 为原本没有参数的 splitter 备选新增一个 `Unit` 假设。此改动修复了 #11211。
...
* [#11261](https://github.com/leanprover/lean4/pull/11261) 延续 #11256，继续统一 matcher 与 splitter。
  其中尤其移除了 `numParams` 是否包含 `discrEqns` 的歧义。

* [#11269](https://github.com/leanprover/lean4/pull/11269) 为空列表和空数组的可判定相等性提供支持。
  同时适当调整列表和数组的可判定相等性实现，使所有 diamond 在定义上相等。
...
* [#11302](https://github.com/leanprover/lean4/pull/11302) 将 CTests 的测试名改为使用文件名。
  ...
  ```
  这样就可以在 VSCode 终端里对它们执行 Ctrl-Click。
...
* [#11422](https://github.com/leanprover/lean4/pull/11422) 在 `grind` 中使用 `Mon.mul` 的一种针对 kernel reduction 优化过的变体。
...
* [#11432](https://github.com/leanprover/lean4/pull/11432) 修复了 `#guard_mgs` 的 docstring 中的一个拼写错误。
...
* [#11490](https://github.com/leanprover/lean4/pull/11490) 防止 `try` 吞掉嵌套 `simp` 调用产生的 heartbeat 错误，更一般地说，也确保 `isRuntime` 标志会被 `throwNestedTacticEx` 传播。
  这避免了证明的行为（尤其是使用 `aesop` 的证明）受当前递归深度或 heartbeat 限制影响。
...
* [#11508](https://github.com/leanprover/lean4/pull/11508) 在对值做匹配时，若无需生成假设（例如存在 catch-all，因此不需要完备性检查），就避免生成这些 hyps。

  这一调整得益于 #11220。

* [#11510](https://github.com/leanprover/lean4/pull/11510) 避免在 caseValues 中运行两次 substCore。

* [#11511](https://github.com/leanprover/lean4/pull/11511) 实现了一个 linter，会在应用已弃用 coercion 时发出警告。
  当 `Option` coercion 或 `Subarray` 到 `Array` 的 coercion 在 `Init` 或 `Std` 中使用时，它也会发出警告。
  当前这个 linter 仅限于 `Coe` 实例；`CoeFun` 等实例不在考虑范围内。
...
* [#11554](https://github.com/leanprover/lean4/pull/11554) 为 Lean 添加 `@[suggest_for]` 标注，使 lean 能为多数默认导入类型（数组、列表、字符串、子串、子数组以及向量）把 `.all` 或 `.any` 方法纠正为 `.every` 或 `.some` 方法。
...
* [#11620](https://github.com/leanprover/lean4/pull/11620) 将 Batteries.WF 迁移到 Init.WFC，用于可执行的良基不动点。
  它引入 `csimp` 定理，用可执行定义替代 recursor 和不可执行定义。
...
* [#11637](https://github.com/leanprover/lean4/pull/11637) 宣布模块系统不再是实验性功能，并把 `experimental.module` 选项变成待删除的 no-op。

* [#11644](https://github.com/leanprover/lean4/pull/11644) 让 `.ctorIdx` 不再是 abbrev；我们不希望 `grind` 展开它。

* [#11645](https://github.com/leanprover/lean4/pull/11645) 修复了 `propagateForallPropUp` 的 docstring；此前只是复制粘贴产物。
...
* [#11657](https://github.com/leanprover/lean4/pull/11657) 在 #11652 的基础上进一步改进，同时保留了针对 kernel reduction 优化过的定义。
...
* [#9302](https://github.com/leanprover/lean4/pull/9302) 修改了 `Option.instDecidableEq` 和 `Option.decidableEqNone`，使后者可以被设为全局实例而不会产生 diamonds。
...
* [#11235](https://github.com/leanprover/lean4/pull/11235) 为 `Lean.Parser.Term.elabToSyntax` 注册了一个 node kind，以支持 `Lean.Elab.Term.elabToSyntax` 的功能，而无需为用户可访问语法单独注册解析器。
...
* [#11242](https://github.com/leanprover/lean4/pull/11242) 大幅修改了 `ToIterator` 类型类的签名。
  ...
  因此，这个 PR 为 `Subarray` 提供了手写迭代器，不需要依赖类型状态，而且比之前更快。
...
* [#11263](https://github.com/leanprover/lean4/pull/11263) 修复了新 `String` API 中的若干内存泄漏。
...
* [#11267](https://github.com/leanprover/lean4/pull/11267) 重命名了 `DHashMap`/`HashMap`/`HashSet`/`DTreeMap`/`TreeMap`/`TreeSet` 上 union 的 congruence 引理，使之符合放在 `Equiv` 命名空间中的约定。

* [#11276](https://github.com/leanprover/lean4/pull/11276) 清理了 `String.find` 周边的 API，并统一迁移到新的位置类型 `String.ValidPos` 和 `String.Slice.Pos`。
...
* [#11341](https://github.com/leanprover/lean4/pull/11341) 添加了从 `String` 到 `String.Slice` 的 coercion。
...
* [#11376](https://github.com/leanprover/lean4/pull/11376) 试图提升 `String.contains`、`String.find` 等函数在使用 `Char` 或 `Char -> Bool` 模式时的性能。
  做法是把 needle 移出迭代器状态，以绕过编译器中缺失的 unboxing。
...
* [#11404](https://github.com/leanprover/lean4/pull/11404) 为 `DTreeMap`/`TreeMap`/`TreeSet` 及其 extensional 变体添加 `BEq` 实例，并证明把它与 hashmap 等价性 / extensional 变体相等性关联起来的引理。

* [#11407](https://github.com/leanprover/lean4/pull/11407) 为 `DTreeMap`/`TreeMap`/`TreeSet` 添加差集操作，并证明若干相关引理。

* [#11421](https://github.com/leanprover/lean4/pull/11421) 为 `DHashMap`/`HashMap`/`HashSet` 及其 extensional 变体添加可判定相等性。

* [#11439](https://github.com/leanprover/lean4/pull/11439) 对 String API 做了小规模维护。

* [#11448](https://github.com/leanprover/lean4/pull/11448) 调整了常量 `DTreeMap`（及相关）查询中的 `Inhabited` 实例位置，例如 `Const.get!`；此时可以在证明 key 之前先提供 `Inhabited` 实例。

* [#11452](https://github.com/leanprover/lean4/pull/11452) 添加了若干引理：若某个 get 操作返回了值，则被查询的 key 必须包含在集合中。
...
* [#11527](https://github.com/leanprover/lean4/pull/11527) 为 `DTreeMap`/`TreeMap`/`TreeSet` 及其 extensional 变体添加可判定相等性。

* [#11528](https://github.com/leanprover/lean4/pull/11528) 为所有 `DTreeMap` 及其派生容器添加引理，将 `minKey?` 和 key 列表上的 `min?` 联系起来。
...
* [#11654](https://github.com/leanprover/lean4/pull/11654) 更新了 `grind` 的 docstring。
...
* [#11247](https://github.com/leanprover/lean4/pull/11247) 修复了 `grind` 预处理器中的一个问题：`simp` 可能会引入已赋值的（universe）元变量（例如执行 zeta 归约时）。
...
* [#11248](https://github.com/leanprover/lean4/pull/11248) 实现了 `revert` 选项，其默认值为 `false`。
  ...
  * 用户反馈 `grind` 会包含不必要的参数。见 [here](https://leanprover.zulipchat.com/#narrow/channel/270676-lean4/topic/Grind.20aggressively.20includes.20local.20hypotheses.2E/near/554887715)。
  * 还会引入不必要的节变量。可见 Sebastian Graf 贡献的新测试。
  * 最后，它还阻碍了我们像 `simp` 那样支持任意参数。在 `simp` 中，我实现了一个模拟局部宇宙多态定理的机制，但这种方法不能用于 `grind`，因为它没有回退（以及重新引入）局部宇宙多态定理的机制。要增加这样的机制需要做大量工作：我需要修改局部上下文对象。我考虑过维护从原始变量到新变量的替换，但这也很棘手，因为映射必须存储在 `grind` 目标对象中，而且它并不只是简单映射。回退完所有内容后，我需要保留一串原始变量，以便在重新引入它们时把它们加入映射，而急切的 case split 会让这件事复杂得多。整个方案显得过于混乱。
...
* [#11268](https://github.com/leanprover/lean4/pull/11268) 实现了对任意 `grind` 参数的支持。
  该特性类似于 `simp` 中的做法：把证明项视为局部 universe-polymorphic 引理。
...
* [#11348](https://github.com/leanprover/lean4/pull/11348) 通过移除 TODO 注释并取消注释该命令，在 `Init.Data.List.Lemmas` 中启用了 `grind_annotated` 命令。
...
* [#11391](https://github.com/leanprover/lean4/pull/11391) 为 `grind_pattern` 命令实现了新类型的约束。
  ...
  由于该变更会影响 `.olean` 格式，因此需要手动执行 `update-stage0`；否则 PR 会失败。
...
* [#11429](https://github.com/leanprover/lean4/pull/11429) 为 `grind_pattern` 命令编写文档，说明如何手动选择定理实例化模式，包括 multi-pattern 以及约束系统（`=/=`、`=?=`、`size`、`depth`、`is_ground`、`is_value`、`is_strict_value`、`gen`、`max_insts`、`guard`、`check`）。
...
* [#11466](https://github.com/leanprover/lean4/pull/11466) 移除了 `exact?` 与 `apply?` 的 “first pass” 行为；它们以前会在库搜索前，先在原始目标上尝试 `solve_by_elim`。
  这简化了 `librarySearch` API，并让这些策略专注于其主要用途：查找库引理。
...
* [#11480](https://github.com/leanprover/lean4/pull/11480) 新增 `grind` 选项 `reducible`（默认：`true`）。
  ...
  该选项只影响定义相等性；规范化器和定理模式内部化 总会展开 reducible 声明，而不受该设置影响。

* [#11481](https://github.com/leanprover/lean4/pull/11481) 修复了 `grind?` 中的一个 bug。
  使用 `grind` 交互模式给出的建议会丢失用户提供的配置选项。
  在下面的说明中，第三条建议会丢掉 `-reducible` 选项。

* [#11484](https://github.com/leanprover/lean4/pull/11484) 修复了 `grind` 模式校验中的一个 bug。该问题会影响那些本身是命题的类型类。
...
* [#11547](https://github.com/leanprover/lean4/pull/11547) 确保 `register_try?_tactic` 创建的辅助定义属于内部实现细节，不应对面向用户的 linter 可见。
...
* [#11579](https://github.com/leanprover/lean4/pull/11579) 确保 ground theorem 会被正确处理为 `grind` 参数。
...
* [#11589](https://github.com/leanprover/lean4/pull/11589) 改进了 `grind` 模式的索引。
  我们现在会把嵌套 ground pattern 中出现的符号也纳入索引。
  这对最小化被激活的 E-match 定理数量很重要。
...
* [#11594](https://github.com/leanprover/lean4/pull/11594) 修复了 `grind?`，使其在建议中包含项参数（例如 `[show P by tac]`）。
  之前这些参数会丢失，因为项参数存储在 `extraFacts` 中，不像具名引理那样通过 E-matching 跟踪。

* [#11604](https://github.com/leanprover/lean4/pull/11604) 修复了 `grind` 处理无参数定理的方式。

* [#11605](https://github.com/leanprover/lean4/pull/11605) 修复了 `grind linarith` 中 `a^p` 项 internalizer 的一个 bug。

* [#11609](https://github.com/leanprover/lean4/pull/11609) 改进了 `grind` 中的 case split 启发式。
  在这个 PR 中，我们不会在第一个分支里增加 case split 计数。
  核心思路是利用 non-chronological backtracking：若第一个分支可由不依赖分支假设的证明解决，我们就回溯并直接关闭原目标。
  在这种场景下，这次 case split 是“免费的”，并未对证明作出贡献。
  不对其计数后，当 case split 最终无关时，我们就能探索得更深。
  新的启发式修复了 #11545 中第二个示例。

* [#11613](https://github.com/leanprover/lean4/pull/11613) 确保我们会把 ring normalizer 应用于从 `grind` core 模块传播到 `grind lia` 的相等式。
...
* [#11635](https://github.com/leanprover/lean4/pull/11635) 确保 `grind` 中使用的模式归一化器不会违反 `Grind.genPattern` 与 `Grind.getHEqPattern` 这些 gadget 所依赖的假设。

* [#11638](https://github.com/leanprover/lean4/pull/11638) 修复了 `grind` 中 bitvector 字面量的 internalization。
...
* [#11658](https://github.com/leanprover/lean4/pull/11658) 修复了 `grind` 中参数化字面量 internalization 的一个 bug，也就是类型为 `BitVec _` 或 `Fin _` 的字面量。
...
* [#11663](https://github.com/leanprover/lean4/pull/11663) 修复了 `grind` 模式校验器。
  它覆盖了实例未用隐式实例 binder 标记的情况。这种情况会出现在如下声明中：
  ```lean
...
* [#11185](https://github.com/leanprover/lean4/pull/11185) 修复了 `reduceArity` 编译器 pass，使其会考虑那些元数已被缩减的函数上的 over-application。
  以前该 pass 假定应用的参数个数始终等于签名中的参数个数。
...
* [#11210](https://github.com/leanprover/lean4/pull/11210) 修复了在处理 #11078 时暴露出的一个 LCNF 化简器缺陷。
  在某些由 `unsafeCast` 引起的场景中，simplifier 会记录关于 `cases` 的错误信息，从而在后续导致更多 bug。

* [#11215](https://github.com/leanprover/lean4/pull/11215) 修复了一个问题：标题嵌套层级在不同 moduledoc 之间能被正确跟踪，但在单个 moduledoc 内部不能。

* [#11217](https://github.com/leanprover/lean4/pull/11217) 修复了 #10982 中 closure allocator 修改的后续影响。
  据我们所知，这个 bug 只有在不使用 mimalloc 的非默认构建配置中才会明显出现，例如：
  位置所有者0
...
* [#11362](https://github.com/leanprover/lean4/pull/11362) 加快了 ElimDeadBranches 编译器 pass 的终止。

* [#11366](https://github.com/leanprover/lean4/pull/11366) 按大小递增顺序排列送入 ElimDeadBranches 的声明。
  当存在大量迭代时，这可以提升性能。

* [#11381](https://github.com/leanprover/lean4/pull/11381) 修复了一个 bug：闭项提取没有遵守 C 发射器的隐式不变量，即在 SCC 内先放 闭项声明、后放其他声明。
  这个 bug 目前尚未在实际使用中触发，但在即将对 specializer 进行修改的工作中被发现了。

* [#11383](https://github.com/leanprover/lean4/pull/11383) 修复了带有 `extern` 标记的 unboxed 参数的结构投影编译问题，并补上缺失的 `dec` 指令。
...
* [#11445](https://github.com/leanprover/lean4/pull/11445) 稍微改进了创建 boxed 声明时涉及的类型。
...
* [#11451](https://github.com/leanprover/lean4/pull/11451) 调整了 LCNF 中的 λ 提升器：若可能则进行 η 收缩，而不是 λ 提升。
  这避免了整个代码库中产生几百个不必要的 lambda。

* [#11517](https://github.com/leanprover/lean4/pull/11517) 为 Nat.mul 实现了常量折叠。

* [#11525](https://github.com/leanprover/lean4/pull/11525) 让 LCNF simplifier 在所有备选都为 `.unreach` 时，将其直接化简为一个 `.unreach`。
...
* [#11364](https://github.com/leanprover/lean4/pull/11364) 为参考手册中出现的常量补上缺失的 docstring。

* [#11472](https://github.com/leanprover/lean4/pull/11472) 为 `mkSlice` 方法补上缺失的 docstring。

* [#11550](https://github.com/leanprover/lean4/pull/11550) 审查了会出现在 Lean 参考手册中的 `Std.Do` docstring，并补上缺失项。

* [#11575](https://github.com/leanprover/lean4/pull/11575) 修复了 `cases` 策略 docstring 中的一个拼写错误。

* [#11595](https://github.com/leanprover/lean4/pull/11595) 记录了 `tests/lean/run/` 中的测试会以 `-Dlinter.all=false` 运行，并解释了在测试 linter 行为时如何启用特定 linter。
...
* [#11164](https://github.com/leanprover/lean4/pull/11164) 确保对未知标识符给出的 code action 会在 `module` 中正确插入 `public` 和/或 `meta`。
...
* [#11216](https://github.com/leanprover/lean4/pull/11216) 确保在 Lake 代码中始终为 `computeArtifact` 提供 `text` 参数，同时顺带修复了 `buildArtifactUnlessUpToDate` 的一个哈希 bug。

* [#11270](https://github.com/leanprover/lean4/pull/11270) 为 Lake 添加模块解析过程，以消除在多个包中定义的模块歧义。

* [#11500](https://github.com/leanprover/lean4/pull/11500) 在构建目标所使用包的名称中加入 workspace-index。
  为了澄清一个包名的不同用途之间的区别，这个 PR 还弃用了 `Package.name`，转而使用更贴合具体用途的变体（例如 `Package.keyName`、`Package.prettyName`、`Package.origName`）。
...
* [#11496](https://github.com/leanprover/lean4/pull/11496) 为 Mathlib 中使用的 `shake` 实现了新的 flag 与标注：
