...
inductive SourceInfo where
  ...
  `leading` 前导空白是在解析完成后由 `Syntax.updateLeading` 推断的，因为解析过程中，
  尤其存在回溯时，“前一个记号”并没有良好定义。
  -/
...
生成它的 `Parser` 或 `ParserDesc` 声明。解析基础设施还使用若干不对应解析器声明的
...
inductive Syntax where
  ...
  和原子的字段中。该字段有两种用途：
   ...
   2. 引用创建的节点用该字段把语法标记为合成语法（存储
      `Lean.SourceInfo.fromRef` 的结果），即使其首尾记号本身不是合成的。
  -/
  ...
  * `preresolved` 是它可能指向的声明列表，由[引用](https://lean-lang.org/doc/reference/4.34.0-rc1/find/?domain=Verso.Genre.Manual.section&name=quasiquotation)填充。
  -/
...
“伪种类”是按约定赋予非 `Syntax.node` 值的种类：`Syntax.ident` 使用 `identKind`，
`Syntax.missing` 使用 `` `missing ``，原子使用其字符串字面量。
-/
...
名称 `` `ident `` 实际上并不用作 `Syntax.node` 值的种类；按约定，它用作
`Syntax.ident` 值的种类。
-/
...
/--
`` `interpolatedStrKind `` 是 `s!"value = {x}"` 中 `"value = {x}"` 这类插值字符串
字面量的节点种类。
-/
abbrev interpolatedStrKind : _root_.Lean.SyntaxNodeKind := _root_.Lean.interpolatedStrKind
/--
`` `interpolatedStrLitKind `` 是 `s!"value = {x}"` 中 `"value = {` 和 `}"` 这类插值
字符串字面量片段的节点种类。
-/
...
解析器优先选择更长的匹配，但最长匹配并不总是唯一的。所有解析结果都会保存起来，
直到有类型信息时再决定使用哪一个。
-/
...
单元素 `SyntaxNodeKinds` 极为常见，可直接写成名称字面量而不是列表；只有空集合或
多元素种类集合才需要列表语法。
-/
...
分隔数组由 `,*` 等重复运算符产生。[强制转换](https://lean-lang.org/doc/reference/4.34.0-rc1/find/?domain=Verso.Genre.Manual.section&name=coercions)
到 `Array (TSyntax ks)` 或从中转换时，会按需插入或移除分隔符。无类型版本是
...
/--
为以字符串提供的数字创建字面量语法。调用者必须确保该字符串是 `num` 记号解析器的
合法记号。
...
/--
为科学计数法数字创建字面量语法。调用者必须确保所提供的字符串是合法的科学计数法
字面量。
