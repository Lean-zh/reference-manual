...
当原本 intended 为声明名称的标识符被误写在绑定项位置时，也可能产生此错误。此时，错误标识符会被视为
...
必须显式指定 `x` 的类型 `α`。注意，如果启用了自动插入隐式参数（默认如此），则无需为 `α` 本身提供
...
def plusTwo (x : Nat) : Nat :=
  x + 2
```
尽管在 `plusTwo` 的定义体中可以推断 `x` 的类型为 `Nat`，但由于定义结果类型（`Nat`）已显式指定，
精译定义类型时无法使用这一信息。仅根据头部信息无法确定 `x` 的类型，于是产生所示错误。因此必须在
其绑定项中包含 `x` 的类型。
:::
