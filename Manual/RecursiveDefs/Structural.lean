...
这个版本不是分支于 `Nat` 的布尔相等测试结果，而是分支于 {tech (key := "propositional equality")}[命题相等] 的可判定性：
...
这个分支会收到一个值以及一张 `below` 表；该表包含对所有比给定值更小的值递归所得的结果，分支应利用表中的内容来满足这个给定值对应的 motive。
...
由于 `table` 的类型依赖于 `x`，因此在用 {name}`half.match_1` 分情况时，它也必须一并泛化，从而得到一个带额外参数的 motive。
...
实际的精译器会在 motive 中插入带新鲜名称的哨兵类型，以此跟踪“当前检查是否结构递归的参数”与“所有较小值表中的位置”之间的对应关系。
:::
